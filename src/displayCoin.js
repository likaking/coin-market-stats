...
export function DisplayCoin({activeCoins,buy,setBuy,setActiveCoins,Speech,setQuickData,setCoinArr,currency,searchGems,openModal,setOpenModal,modalSearch,setModalSearch,startModal,setStartModal,index,setIndex,priceAlertzPing,setPriceAlertzPing,currencySymbol,setCurrencySymbol}){
...
<div className={styles.dipedCrypto_HR}><div className={styles.dipedCrypto_HR_price}>Price</div> <div className={styles.dipedCrypto_HR_hourly}>1h % </div> <div className={styles.dipedCrypto_HR_24H}>24h %</div> <div className={styles.dipedCrypto_HR_7dh}>7d %</div> <div className={styles.dipedCrypto_HR_mktCap}>Market Cap</div> <div className={styles.dipedCrypto_HR_vol}>Volume (24)</div>  <div className={styles.dipedCrypto_HR_circulatingSup}>Circulating Supply</div> <div className={styles.dipedCrypto_HR_sparkLine}>Last 7 days</div> </div>
{
    ...
    <div className={styles.dipedCrypto_R_container_hourlyChange} style={{color: itemz.price_change_percentage_1h_in_currency.toString().startsWith("-") ?'red' : 'rgb(28, 187, 44)'}}><FaCaretDown style={{display: itemz.price_change_percentage_1h_in_currency.toString().startsWith("-") ?'block' : 'none' }}  className={styles.dipedCrypto_R_faCaretUp} /> <FaCaretUp style={{display: itemz.price_change_percentage_1h_in_currency.toString().startsWith("-") ?'none' : 'block' }} className={styles.dipedCrypto_R_faCaretUp} />{itemz.price_change_percentage_1h_in_currency < 1 && <span>{Number(itemz.price_change_percentage_1h_in_currency.toString().replace('-','')).toFixed(2)}</span>}  {itemz.price_change_percentage_1h_in_currency > 1 &&<span>{(itemz.price_change_percentage_1h_in_currency.toString().replace('-','')).toFixed(2)}</span>}%</div>
    <div className={styles.dipedCrypto_R_container_24hChange}  style={{color: itemz.price_change_percentage_24h_in_currency.toString().startsWith("-") ?'red' : 'rgb(28, 187, 44)'}}> <FaCaretDown style={{display: itemz.price_change_percentage_24h_in_currency.toString().startsWith("-") ?'block' : 'none' }} className={styles.dipedCrypto_R_faCaretUp} /> <FaCaretUp style={{display: itemz.price_change_percentage_24h_in_currency.toString().startsWith("-") ?'none' : 'block' }} className={styles.dipedCrypto_R_faCaretUp} />{itemz.price_change_percentage_24h_in_currency < 1 && <span>{Number(itemz.price_change_percentage_24h_in_currency.toString().replace('-','')).toFixed(2)}</span>}  {itemz.price_change_percentage_24h_in_currency > 1 &&<span>{Number(itemz.price_change_percentage_24h_in_currency.toString().replace('-','')).toFixed(2)}</span>}%</div>
    <div className={styles.dipedCrypto_R_container_7d}  style={{color: itemz.price_change_percentage_7d_in_currency.toString().startsWith("-") ?'red' : 'rgb(28, 187, 44)'}}><FaCaretDown style={{display: itemz.price_change_percentage_7d_in_currency.toString().startsWith("-") ?'block' : 'none' }} className={styles.dipedCrypto_R_faCaretUp}/> <FaCaretUp style={{display: itemz.price_change_percentage_7d_in_currency.toString().startsWith("-") ?'none' : 'block' }} className={styles.dipedCrypto_R_faCaretUp} />{itemz.price_change_percentage_7d_in_currency < 1 && <span>{Number(itemz.price_change_percentage_7d_in_currency.toString().replace('-','')).toFixed(2)}</span>}  {itemz.price_change_percentage_7d_in_currency > 1 &&<span>{Number(itemz.price_change_percentage_7d_in_currency.toString().replace('-','')).toFixed(2)}</span>}%</div>
	<div className={styles.dipedCrypto_R_container_mktCap}>{currencySymbol}{itemz.market_cap.toLocaleString()}</div>
    <div className={styles.dipedCrypto_R_container_volume}>{currencySymbol}{itemz.total_volume.toLocaleString()}</div>
	<div className={styles.circulatingSupply}>{itemz.total_volume.toLocaleString()} {itemz.symbol.toUpperCase()}</div>
